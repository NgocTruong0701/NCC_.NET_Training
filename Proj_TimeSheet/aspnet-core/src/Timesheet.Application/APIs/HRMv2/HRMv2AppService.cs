...
namespace Timesheet.APIs.HRMv2
{
    public class HRMv2AppService : AppServiceBase
    {
        ...
        public List<ChamCongInfoDto> GetChamCongInfo(InputCollectDataForPayslipDto input)
        {
            ...
            return WorkScope.GetAll<MyTimesheet>()
                .Select(s => new { s.Status, DateAt = s.DateAt.Date, s.WorkingTime, s.TypeOfWork, s.User.NormalizedEmailAddress })
                ...
                .Select(x => new ChamCongInfoDto
                {
                    ...
                    NormalWorkingDates = x.ListDate.Where(s => s.Date.DayOfWeek != DayOfWeek.Saturday).Where(s => s.WorkingMinute > 0)
                    .Select(s => s.Date).ToList()
