...
namespace Timesheet.Timesheets.Timesheets
{
    ...
    public class TimesheetAppService : AppServiceBase
    {
        ...
        public async Task<List<MyTimeSheetDto>> GetAll(DateTime? startDate, DateTime? endDate, TimesheetStatus status, long? projectId, HaveCheckInFilter? checkInFilter)
        {
        ...
        public async Task<List<TimeSheetWarningDto>> GetTimesheetWarning(long[] myTimesheetIds)
        {
           ...
           .Select(s => s.DayOff).ToListAsync();
            ...
            if (listTimesheetWarningByUserDateAt.Count() != 0)
            {
                ...
                return result.OrderByDescending(x => x.DateAt)
                    .ToList();

            }

            return resultListTimesheetWarningByUserDateAt.OrderByDescending(x => x.DateAt).ToList();
        }
        ...
        public async Task<object> GetQuantiyTimesheetStatus(DateTime? startDate, DateTime? endDate, long? projectId, HaveCheckInFilter? checkInFilter)
        {
            var projectIds = await WorkScope.GetAll<ProjectUser>()
                .Where(s => s.UserId == AbpSession.UserId.Value && s.Type == ProjectUserType.PM)
                .Where(s => !projectId.HasValue || s.ProjectId == projectId)
