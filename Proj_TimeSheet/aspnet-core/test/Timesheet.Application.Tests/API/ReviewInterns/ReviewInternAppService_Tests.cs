...
namespace Timesheet.Application.Tests.API.ReviewInterns
{
    public class ReviewInternAppService_Tests : TimesheetApplicationTestBase
    {
        ...
        public ReviewInternAppService_Tests()
        {
            ...
            _reviewInternAppService = new ReviewInternAppService(_backgroundJobManager, _hRMv2Service, _workScope, _projectService, _hRMService);
        ...
        public async Task Should_Approve_All_Review_Detail()
        {
            ...
            await WithUnitOfWorkAsync(async () =>
            {
                await _reviewInternAppService.ApproveAll(reviewInternId);
            });

            await WithUnitOfWorkAsync(async () =>
            {
                var details = _workScope.GetAll<ReviewDetail>().Where(x => x.ReviewId == reviewInternId);
